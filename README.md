...
### 당신의 달콤한 감정일기! :ledger::sparkles:

### 마시멜로 일기와 함께 달콤한 하루 기록을 시작하세요! :candy::lollipop:
...
- [프로젝트 개요](###프로젝트-개요)
- [팀원 소개](###팀원-소개)
- [와이어 프레임 및 디자인 컨셉](###와이어 프레임 및 디자인 컨셉)
- [ERD](###ERD)
- [REST API](###REST-API)
- [Git 협업 컨벤션](###Git-협업-컨벤션)
- [기술 스택 및 아키텍쳐](###기술-스택-및-아키텍쳐)
- [파일 구조](###파일-구조)
- [포팅 메뉴얼](###포팅-메뉴얼)
- [서비스 화면](###서비스-화면)
- [UCC](#star2-UCC)
...
- 매일 기록한 일기가 어떤 감정들로 구성되어있는지 알려주고, 사용자는 해당 결과를 기반으로 노래를 추천받을 수 있다. 
- 직관적이면서도 감성을 살린 귀여운 UI를 통해 사용자 접근성을 높이고 일기 쓰는 즐거움을 더했다.
- 감정 분석 레포트를 통해 월별 감정 비율 분석 그래프를 제공한다.
- 선택한 기간에 작성한 일기 중 가장 긍정적인 감정으로 이루어진 긍정일기로 다시 안내하는 리마인드 기능을 통해, 과거의 일기를 다시보며 추억할 수 있다.
- 당일 일기 분석 결과와 레포트를 이미지로 공유하고 저장 가능하다.
- 원하는 일기를 검색할 수 있는 검색 기능을 제공한다.
- 개인 프라이버시 보호를 위한 비밀번호 설정 기능을 제공한다.
...
- 개발 전 충분한 회의를 거쳐 구현할 핵심 기능 및 캐릭터 디자인, 색상팔레트를 선정하여 추후 개발과정에서 디자인이 크게 변동되어 발생할 수 있는 번거로움과 혼동을 줄였다.
...
- 팀 노션 문서를 통해 FE와 BE간의 원활한 확인 및 작업이 가능하도록 했다.
- **REST API 퍼블릭 링크**
